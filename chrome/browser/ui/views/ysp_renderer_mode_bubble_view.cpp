...
public:
	...
	void Paint(gfx::Canvas* canvas, views::View* view) const override
	{
		...
		// views::Button::ButtonState state = button ? button->state() : views::Button::STATE_NORMAL;
...
void RendererModeBubbleView::ShowBubble(views::View * anchor_view, const gfx::Rect & anchor_rect, 
	gfx::NativeView parent_window, Browser* browser,
	const GURL & url,RendererMode renderer_mode)
{
	if (pRendererModeBubbleView)
		return;

	pRendererModeBubbleView = new RendererModeBubbleView(anchor_view, browser,url, renderer_mode);
	if (!anchor_view) 
	{
...
{
	pBlinkButton = new views::LabelButton(this, l10n_util::GetStringUTF16(IDS_CHROME_CORE));
	gfx::ImageSkia* blinkImg = ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_YSP_BLINK_CORE);
	...
	pSysIEButton = new views::LabelButton(this, l10n_util::GetStringUTF16(IDS_IE_CORE));
	gfx::ImageSkia* sysIeImg = ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_YSP_IE_CORE);
	...
	cs->AddColumn(views::GridLayout::FILL, views::GridLayout::FILL, 0, views::GridLayout::USE_PREF,0, 0);
...
void RendererModeBubbleView::ButtonPressed(views::Button * sender, const ui::Event & event)
{
	if (pBrowser == NULL)
		return;
	GURL ysp_url;
...
void RendererModeBubbleView::SetButtonSelected(views::LabelButton * pSelectButton)
{
	if (pSelectButton==NULL)
		return;
	gfx::ImageSkia* img = NULL;
	if (pSelectButton == pBlinkButton)
		img = ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_YSP_BLINK_CORE_WHITE);
	else
		img = ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(IDR_YSP_IE_CORE_WHITE);
	pSelectButton->SetImage(views::Button::STATE_NORMAL, *img);
	// views::Background* backGround = views::Background::CreateSolidBackground(55, 159, 255, 255);
	// pSelectButton->set_background(backGround);
}

RendererModeBubbleView::RendererModeBubbleView(views::View * anchor_view, Browser* browser, const GURL & url, RendererMode mode)

...
window.HomeProxy = HomeProxy

HomeProxy.prototype = function () {

    var LOGINSTATUS = null;  // 保留当前用户登录状态
    var CONFIGDATA = {}; //  保留当前用户的配置信息
    var USERINFO = {}; // 保留当前用户的用户信息
    ...
    var _getLoggingStatusCallback = function (loggingStatus, msg) {
        console.log('log status', loggingStatus)
    ...
    // 获取登录CONFIGDATA完成--浏览器回调方法
    var _getConfigDataCallback = function (configData) {
        var ret = {};
        if (CONFIGDATA.companyTitle != configData.companyTitle)
            ret.companyTitle = true;
        else
            ret.companyTitle = false;

        if (CONFIGDATA.iconUrl != configData.iconUrl)
            ret.iconUrl = true;
        else
            ret.iconUrl = false;

        if (CONFIGDATA.backgroundUrl != configData.backgroundUrl)
            ret.backgroundUrl = true;
        else
            ret.backgroundUrl = false;

        if (CONFIGDATA.groupList != configData.groupList)
            ret.groupList = true;
        else
            ret.groupList = false;

        if (ret.companyTitle || ret.iconUrl || CONFIGDATA.groupList) {
            CONFIGDATA = configData;
            BROWSERNOTIFICATION.fireNotification({ type: "COMPANY_INFO_NOTIFICATION", return: ret });
        }
    };

    // 获取设备信息完成--浏览器回调方法
    var _getUserInfoCallback = function (ret) {
        BROWSERNOTIFICATION.fireNotification({ type: "GET_USER_INFO_NOTIFICATION", return: ret });
    };
    ...
    var _registNotification = function (type, target, handler) {
	    try {
	        if (type == "LOGGING_STATUS_NOTIFICATION" ||
                type == "COMPANY_INFO_NOTIFICATION" ||
                type == "GET_USER_INFO_NOTIFICATION" ||
	...
	var proxyCallBack = {
        ...
        getConfigDataCallback      : _getConfigDataCallback,
	    // 从浏览器得到用户信息--浏览器回调方法
        getUserInfoCallback        : _getUserInfoCallback,
	    // 设置自动登录状态完成时浏览器回调此方法
	...
	return {
        ...
        // 上层调用此方法从缓存中获取公司信息
	    getCompanyInfoFromCache: function () { return { iconUrl: CONFIGDATA.iconUrl, companyTitle: CONFIGDATA.companyTitle, backgroundUrl: CONFIGDATA.backgroundUrl }; },
	    // 上层调用此方法从缓存中获取应用信息
	    getAppGroupListFromeCache: function () { return CONFIGDATA.groupList; },
	    ...
	    // 上层调用此方法获得用户信息， 调用完成后触发GET_USER_INFO_NOTIFICATION通知。
        getUserInfo: function () { HOMEBROWSER.command("getParam", ["userInfo"]); },
	    // 上层调用此方法更新HomeProxy中的公司信息Cache，  调用完成后触发COMPANY_INFO_NOTIFICATION通知。
        updateCompanyInfoCache: function () { HOMEBROWSER.command("getParam", ["configData"]) },
	    // 上层调用此方法注册指定的通知
        // 目前支持的通知事件有：
	    //     LOGGING_STATUS_NOTIFICATION
	    //     COMPANY_INFO_NOTIFICATION
	    //     GET_USER_INFO_NOTIFICATION

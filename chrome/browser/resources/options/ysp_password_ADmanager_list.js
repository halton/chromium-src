...
cr.define('options.passwordADManager', function() {
  ...
  /** @const */ var APP_NAME_FIELD = 'appName';
  ...
  /** @const */ var ICON_URL_FIELD = 'iconUrl';
  ...
  PasswordListItem.prototype = {
    ...
    decorate: function() {
      ...
      //    'origin/' + this.url, 16);
	  urlDiv.style.backgroundImage = imageset(this.iconUrl);

      this.urlLink = createUrlLink(this, urlDiv);
      var appNameSpan = this.ownerDocument.createElement('span');
      appNameSpan.className = 'appName-span';
      appNameSpan.innerHTML = this.appName.split('').reverse().join('');
      // appNameSpan.innerHTML = '1111';

      urlDiv.appendChild(this.urlLink);
      urlDiv.appendChild(appNameSpan);
      ...
      usernameInput.className = 'inactive-item appUsername';
      ...
      if (this.federation) {
        ...
        passwordInput.className = 'inactive-item  appPassword';
    ...
    selectionChanged: function() {
      ...
      if (this.selected) {
		...
		// 失去焦点 保存修改内容
		var appUsernames = document.getElementsByClassName("appUsername");
		var appPasswords = document.getElementsByClassName("appPassword");
		if (appUsernames.length > 0 || appPasswords.length > 0) {
		  for (var i = 0; i < appUsernames.length; i ++) {
			appList[i].username = appUsernames[i].value || '';          
		  }
		  for (var i = 0; i < appPasswords.length; i ++) {
			appList[i].password = appPasswords[i].value || '';       
		  }
		  chrome.send('saveADPasswordLists', appList);
		}
        if (button) {
    ...
    onClick_: function(event) {
      if (this.passwordField.type == 'password') {
        ...
        PasswordADManager.requestShowADPassword(this.getOriginalIndex_());
      } else {
        this.hidePassword();
      }
    },
    ...
    // AD 系统名称
    get appName() {
      return this.dataItem[APP_NAME_FIELD];
    },
    set appName(appName) {
      this.dataItem[APP_NAME_FIELD] = appName;
    },
	
	// icon URL
    get iconUrl() {
      return this.dataItem[ICON_URL_FIELD];
    },
    set iconUrl(iconUrl) {
      this.dataItem[ICON_URL_FIELD] = iconUrl;
    },
    /**
   ...
   */
  var ADPasswordsList = cr.ui.define('list');

  ADPasswordsList.prototype = {
    ...
    deleteItemAtIndex: function(index) {
      ...
      PasswordADManager.removeSavedPassword(index);
    },
  ...
  PasswordExceptionsList.prototype = {
    ...
    /** @override */
    deleteItemAtIndex: function(index) {
      PasswordADManager.removePasswordException(index);
    },
  ...
  return {
    ...
    ADPasswordsList: ADPasswordsList,

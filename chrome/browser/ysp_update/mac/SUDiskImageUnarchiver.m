...
{
    NSLog(@"unarchiveWithCompletionBlock enter");
#if 0
...
#else
    SUUnarchiverNotifier *notifier = [[SUUnarchiverNotifier alloc] initWithCompletionBlock:completionBlock
...
{
  NSLog(@"extractDMGWithNotifier enter");
  @autoreleasepool {
    ...
    NSString *toPathRoot = [self.archivePath stringByDeletingPathExtension];
    if (![manager fileExistsAtPath:toPathRoot] &&
        ![manager createDirectoryAtPath:toPathRoot
            withIntermediateDirectories:NO
                             attributes:nil
                                  error:nil]) {
        NSLog(@"Error: %@ doesn't exist", toPathRoot);
    }

    for (NSString *item in contents)
    {
      NSString *fromPath = [mountPoint stringByAppendingPathComponent:item];
      NSString *toPath   = [toPathRoot stringByAppendingPathComponent:item];

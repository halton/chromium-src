...
bool cryptokeyimpl::EncryptString(const std::string& plaintext, std::string* ciphertext)
{
  AES_KEY aes_key;
  char key_data[17] = { 0 };
...
bool cryptokeyimpl::DecryptString(const std::string& ciphertext, std::string* plaintext)
{
  AES_KEY aes_key;
  char key_data[17] = { 0 };
  memcpy(key_data, crypto_impl_key, AES_BLOCK_SIZE);
...
void Des_EncryptString(std::string *ciphertext, const std::string plaintext)
{
    char key[DES_BLOCK_SIZE] = "";
    size_t out_size = plaintext.length();
...
void Des_DecryptString(std::string *plaintext, const std::string ciphertext)
{
  char key[DES_BLOCK_SIZE] = "";
  size_t out_size = ciphertext.size();
    std::string result;

  memcpy(key, cryptokey::pwd, DES_BLOCK_SIZE);
    Des_SetKey(key);
    Des_Dec(&result, ciphertext, out_size);
  size_t length = result.length();
  int offset = result.c_str()[length - 1];
  plaintext->clear();
    plaintext->assign(result, 0, length - offset);
  DLOG(INFO) << "result: [" << result << "] [" << plaintext << "]";
}
...
void sms4_crypt_ecb(sms4_key_t *key, int mode, int length, std::string input, std::string *output);
...
void sms4_crypt_ecb(sms4_key_t *key, int mode, int length, std::string input, std::string *output)
{
  ...
  for (int i = 0; length > 0; length -= SMS4_BLOCK_SIZE) {
    sms4_encrypt((unsigned char *)input.c_str() + i, (unsigned char *)outtmp.c_str(), key);
    i += SMS4_BLOCK_SIZE;
...
*/
void Sms4_EncryptString(std::string *ciphertext, const std::string plaintext)
{
  unsigned char key[SMS4_BLOCK_SIZE] = "";
  sms4_key_t ctx;
  std::string result;
  char padding[SMS4_BLOCK_SIZE] = "";
  size_t sms4_len = plaintext.length();
...
*/
void Sms4_DecryptString(std::string *plaintext, const std::string ciphertext)
{
  unsigned char key[SMS4_BLOCK_SIZE] = "";
  sms4_key_t ctx;
  std::string result;
  if (ciphertext.empty()) {
    DLOG(INFO) << "ciphertext is NULL !";
    return;
  }
  size_t sms4_len = ciphertext.length();

  memcpy(key, cryptokey::pwd, SMS4_BLOCK_SIZE);
  sms4_set_decrypt_key(&ctx, key);
  sms4_crypt_ecb(&ctx, DECRYPT, sms4_len, ciphertext, &result);
  size_t length = result.length();
  int offset = result.c_str()[length - 1];
  plaintext->clear();
  plaintext->assign(result, 0, length - offset);
...
YSPCryptoCSingleton* YSPCryptoCSingleton::m_pInstance = nullptr;
YSPCryptoCSingleton * YSPCryptoCSingleton::GetInstance() {
    if(m_pInstance == nullptr)  //判断是否第一次调用
        m_pInstance = new YSPCryptoCSingleton();
    return m_pInstance;
}

void YSPCryptoCSingleton::SetShouldEncrypt() {
  should_encrypt = true;
}
bool YSPCryptoCSingleton::GetShouldEncrypt() {
  return should_encrypt;
}

void YSPCryptoCSingleton::Init(std::string crypto_key) {
  if (crypto_key.empty())
    return;
  ...
  //DLOG(INFO) << "cryptokey::pwd:" << cryptokey::pwd << " cryptokey::iv" << cryptokey::iv;
}

std::string YSPCryptoCSingleton::EncryptString(const std::string plaintext) {
    std::string ciphertext, result, key = "";
  if (plaintext.empty())
    return result;
  key = cryptokey::flags;
  ...
  if (cryptokey::flags == '1') {
    ...
    else if(cryptokey::flags == crypto_sign::cAES || cryptokey::flags == crypto_sign::caes) {
        CookieCryptor Aes_Crypted_;
        Aes_Crypted_.EncryptString(plaintext, &result);
    } else if(cryptokey::flags == crypto_sign::cDES || cryptokey::flags == crypto_sign::cdes) {
        std::string enc_value = plaintext;
        Des_EncryptString(&result, enc_value);
    } else if(cryptokey::flags == crypto_sign::cSM4 || cryptokey::flags == crypto_sign::csm4) {
        Sms4_EncryptString(&result, plaintext);
    } else {
    result.assign(plaintext);
    }
  cryptokeyimpl AES_crypto;
  AES_crypto.EncryptString(key, &ciphertext);
  std::string base64_result = "", base64_ciphertext = "";
  base::Base64Encode(result, &base64_result);
  base::Base64Encode(ciphertext, &base64_ciphertext);
  base64_ciphertext += "[[]]" + base64_result;
    return base64_ciphertext;
}
std::string YSPCryptoCSingleton::DecryptString(const std::string ciphertext) {
    std::string plaintext, cip_result, base64_cip_result;
  if (ciphertext.empty())
    return plaintext;
  size_t offset = ciphertext.find("[[]]");
  if (ciphertext.find("[[]]") != std::string::npos) {
    std::string enc_Key, pla_enc_Key, base64_enc_key;
    enc_Key.assign(ciphertext, 0, offset);
    cryptokeyimpl AES_crypto;
    base::Base64Decode(enc_Key, &base64_enc_key);
    AES_crypto.DecryptString(base64_enc_key, &pla_enc_Key);
    Init(pla_enc_Key);
    cip_result.assign(ciphertext, offset + 4, ciphertext.length() - offset - 4);
  ...
  if (cryptokey::flags == '1') {
    return ciphertext;
        ...
        CookieCryptor Aes_Crypted_;
        Aes_Crypted_.DecryptString(base64_cip_result, &plaintext);
    } else if(cryptokey::flags == crypto_sign::cDES || cryptokey::flags == crypto_sign::cdes) {
        Des_DecryptString(&plaintext, base64_cip_result);
    } else if(cryptokey::flags == crypto_sign::cSM4 || cryptokey::flags == crypto_sign::csm4) {
        Sms4_DecryptString(&plaintext, base64_cip_result);
    } else {
        plaintext.assign(base64_cip_result);
    }
    return plaintext;
}

void YSPCryptoCSingleton::SetUserId(const std::string id)
{
  uuid = id;
}

std::string YSPCryptoCSingleton::GetUserId()
{
  return uuid;
}

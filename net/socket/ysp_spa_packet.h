...
public:
	...
	int InitValues(const std::string& device_id, const std::string& username,
...
private:
	int CreateRandAndTimeStamp(const int timeDiff);
	bool EncryptString(const std::string& plaintext, std::string* ciphertext);
	bool DecryptString(const std::string& ciphertext, std::string* plaintext);
	bool HMACEncryptString(const std::string& plaintext, std::string* ciphertext);

	//encoded_msg: rand_value:base64(device_id):base64(username):timestamp:version:message_type:base64(access_cmd):nat_access:server_auth:client_timeout
	...
	std::string hmac_sha256_encrypted_string_;	//AES-CBC加密串再进行HMAC-SHA256加密

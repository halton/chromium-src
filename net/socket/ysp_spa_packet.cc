...
void InitHmacKey(unsigned char* inner_key,
                 unsigned char* outer_key,
...
int YSPRedcoreSpaPacket::InitValues(const std::string& device_id,
                                    ...
                                    int time_diff) {
  int ret = 0;
  if (device_id.empty() || username.empty() || access_cmd.empty() ||
      server_ip.empty() || key.empty() || hmac_key.empty()) {
  ...
  DLOG(INFO) << "device_id: " << device_id << " b64_tmp: " << b64_tmp
             << " b64_device_id: " << b64_device_id_;
  b64_tmp = "";
  ...
  DLOG(INFO) << "access_cmd: " << access_cmd << " b64_tmp: " << b64_tmp
             << " b64_access_cmd_: " << b64_access_cmd_;
  ...
  {  //验证字符串拼接
    ...
    snprintf(enc_msg + offset, 1024 - offset,
             ":%i:", 1);  // message type: Access message
  ...
  {  //"encoded_string_:sha256_encoded_string_" AES-CBC加密
    ...
    plain_text += encoded_string_ + ":" + sha256_encoded_string_;
    std::string cipher_text = "";
    std::string ciphertmp = "";
    if (!EncryptString(plain_text, &ciphertmp)) {
      DLOG(INFO) << "enctypt failure !";
      return -1;
    }
    char value_tmp[9] = {0};
    get_random_data((unsigned char*)value_tmp, 8);
    cipher_text.assign("Salted__");
    cipher_text.append(value_tmp);
    cipher_text.append(ciphertmp);
    std::string b64_ciphertext = "";
    base::Base64Encode(cipher_text, &b64_ciphertext);
  ...
  {  // AES-CBC加密后的字符串再进行HMAC-SHA256加密
    ...
    memcpy(message_tmp + 64, encrypted_string_.c_str(),
           encrypted_string_.length());
    ...
    hmac_sha256_encrypted_string_.assign(b64_hmac_tmp, 0,
                                         b64_hmac_tmp.find('='));
    DLOG(INFO) << "hmac_sha256_encrypted_string_: "
               << hmac_sha256_encrypted_string_;
  }
...
int YSPRedcoreSpaPacket::CreateRandAndTimeStamp(int time_diff) {
  unsigned long seed;
  ...
  time_stamp_ = base::Time::Now().ToTimeT() + time_diff;

  return 0;
}

bool YSPRedcoreSpaPacket::EncryptString(const std::string& plain_text,
                                        std::string* cipher_text) {
  AES_KEY aes_key;
  char key_data[33] = {0};
...
bool YSPRedcoreSpaPacket::DecryptString(const std::string& cipher_text,
                                        std::string* plain_text) {
  AES_KEY aes_key;
  char key_data[33] = {0};
  memcpy(key_data, block_key_.c_str(), 16);
...
bool YSPRedcoreSpaPacket::HMACEncryptString(const std::string& plain_text,
                                            std::string* cipher_text) {
  if (plain_text.empty())
    return false;
  ...
  if (hmacSha256.Init(hmac_key_)) {
    if (hmacSha256.Sign(plain_text, digestSha256, SHA256_DIGEST_LENGTH)) {

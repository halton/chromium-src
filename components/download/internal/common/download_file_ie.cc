...
DownloadFileIE::DownloadFileIE(
    ...
    : file_(download::DownloadItem::kInvalidId),
      default_dir_(default_downloads_directory),
      download_dest_observer_(observer),
      ie_downloader_(download_ptr),
      self_(this) {}
...
void DownloadFileIE::IE_Initialize(const InitializeCallback& callback) {
  ...
  //     base::Bind(&IEDownloader::RegisterCallback, ie_downloader_,
  //       base::Bind(&DownloadFileIE::OnDataReceived,
  //       self_.GetWeakPtr())));
...
void DownloadFileIE::Initialize(
    InitializeCallback initialize_callback,
    const CancelRequestCallback& cancel_request_callback,
    const download::DownloadItem::ReceivedSlices& received_slices,
    bool is_parallelizable) {}
void DownloadFileIE::AddInputStream(
    std::unique_ptr<download::InputStream> stream,
    int64_t offset,
    int64_t length) {}
void DownloadFileIE::RenameAndUniquify(
    const base::FilePath& full_path,
    const RenameCompletionCallback& callback) {
  RenameWithRetryInternal(full_path, UNIQUIFY, kMaxRenameRetries,
                          base::TimeTicks(), callback);
}

void DownloadFileIE::RenameAndAnnotate(
    const base::FilePath& full_path,
    const std::string& client_guid,
    const GURL& source_url,
    const GURL& referrer_url,
    const DownloadFileIE::RenameCompletionCallback& callback) {
  RenameWithRetryInternal(full_path, ANNOTATE_WITH_SOURCE_INFORMATION,
                          kMaxRenameRetries, base::TimeTicks(), callback);
}
...
void DownloadFileIE::Cancel() {
  file_.Cancel();
  // content::BrowserThread::PostTask(content::BrowserThread::UI, FROM_HERE,
  //   base::Bind(&IEDownloader::FinishDownload,ie_downloader_,true));
}
...
void DownloadFileIE::OnDataReceived(int status, const char* data, int64_t len) {
  ...
  if (status == (int)DOWNLOADINGDATA && data && len > 0) {
    ...
    //     download_dest_observer_, hash));

    // content::BrowserThread::PostTask(content::BrowserThread::UI, FROM_HERE,
    //   base::Bind(&IEDownloader::FinishDownload, ie_downloader_, false));
    ...
    //     download_dest_observer_, DOWNLOAD_INTERRUPT_REASON_NETWORK_FAILED));
...
void DownloadFileIE::RenameWithRetryInternal(
    const base::FilePath& full_path,
    RenameOption option,
    int retries_left,
  ...
  if (ShouldRetryFailedRename(reason) && file_.in_progress() &&
    ...
    // content::BrowserThread::PostDelayedTask(content::BrowserThread::IO,
    // FROM_HERE,
    ...
    //     time_of_first_failure, callback),
...
bool DownloadFileIE::ShouldRetryFailedRename(
    download::DownloadInterruptReason reason) {
  return reason == download::DOWNLOAD_INTERRUPT_REASON_FILE_TRANSIENT_ERROR;
}

base::TimeDelta DownloadFileIE::GetRetryDelayForFailedRename(
    int attempt_number) {
...
void DownloadFileIE::SendUpdate() {
  ...
  //                     download_dest_observer_, file_.bytes_so_far(),
